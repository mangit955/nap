...
- **Anything from a model that reaches a command line must be shell-quoted.** Commit messages are the live case (`shellQuote` in `packages/sandbox/src/git.ts`), and the attacker is whatever the user typed into a chat box. Quoting is verified by round-tripping hostile inputs through a real `sh` in the unit tests — asserting on the *shape* of the quoted string does not distinguish safe from unsafe, because the payload text is present either way.
