...
|---|---|---|
| `Runtime` | Turn lifecycle: acquire sandbox → build context → run agent → persist → publish → commit → verify → snapshot → photograph. Budgets, cancellation, recovery. Opening and closing the **job** a turn belongs to, and arbitrating the turn's claim against the project's own checks (ADR-0006) | Prompt content, model params, tool implementations. Deciding *which* checks exist or how to run one — that is `@nap/verify`'s |
