...
| M2-5 | Sandbox-proxy tools | M1-1, M0-3 | DONE | `packages/agent/src/tools/` — `definitions.ts` (Zod → JSON Schema, never hand-written beside it), `diff.ts` (jsdiff), `execute.ts`. **Emission order is the contract:** `tool.call` → any `file.changed`/`command.output` → `tool.result`, and nothing throws — a thrown tool ends a turn over something the model could have fixed. **`shellQuote` moved to `packages/shared/src/shell.ts`**: `search_files` interpolates a model-written grep pattern, which is the same injection surface commit messages already had, and two copies would be two chances to get it wrong. Its real-`sh` round-trip tests moved with it. `run_command` is deliberately *not* quoted (that would turn a pipeline into a filename) and a **non-zero exit stays `ok: true`** — a failing build is data the model must read, not a broken tool; only a `SandboxError` is an error. `search_files` maps grep's exit 1 to a successful empty search. Output is capped at 30k chars: it lands in jsonb *and* in every later request of the turn. `DistributiveOmit` in `event-store.ts` is now exported, per the CLAUDE.md note. **`@nap/sandbox` is a devDependency of `@nap/agent`** so tests use the real fake, as §3 and the task spec ask; `test/architecture.ts` reads only `dependencies` so it stays green — this diverges from `@nap/context`'s local stub on purpose, because these tools need a working filesystem rather than a read-only shape. Seven guards verified by breaking them, including that the comments check reaches the new nested directory. |
